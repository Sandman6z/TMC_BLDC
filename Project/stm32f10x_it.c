	 ...
	 if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET)
      {
          TIM_ClearITPendingBit(TIM2, TIM_IT_Update);
		  if(timeout)timeout--;
		  if(timeout_18B20)timeout_18B20--;
	      TIMEcount++;
	      ...
	      if(TIMEcount >= TIME_SEC_CYCLE)
	      {
		        ...
		        else
		        {
		        	GPIO_ResetBits(GPIOA,GPIO_Pin_11);
						...
						FAN_SPEED_S = TIM_GetCounter(TIM1);	//风扇转速获取			
...
void TIM3_IRQHandler(void)
{
	if (TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET)
    {
        ...
        if(ms_timeout!=0)ms_timeout--;	
	}
}

/***********************************************************
*函数功能: SPI2中断函数，向PCM1770发送音频数据
*输    入：无
*输    出：无
*作    者: 
*建立日期: 2011.10.11
...
/*  if(USART_GetITStatus(UART5, USART_IT_TXE) != RESET)
  ...
  {
	 i = USART_ReceiveData(UART5);							// 读入数据
  }
}
/******************************************************************************/
/*            STM32F10x Peripherals Interrupt Handlers                        */
...
/*  if(USART_GetITStatus(USART1, USART_IT_TXE) != RESET)
  ...
  {
	 i = USART_ReceiveData(USART1);							// 读入数据
  }
}
...
/*  if(USART_GetITStatus(UART4, USART_IT_TXE) != RESET)
  ...
  {
	 	i = USART_ReceiveData(UART4);							// 读入数据
...
/*  if(USART_GetITStatus(UART4, USART_IT_TXE) != RESET)
  ...
  {
	 	R_Data2 = USART_ReceiveData(USART2);							// 读入数据
	 	if(nRec2 == 0)
		{
			if(R_Data2 == HEAD1)
			{
